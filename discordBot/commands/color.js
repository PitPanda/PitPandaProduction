...
const command = async (msg,rest) => {
    ...
    if(!(/^#?[0-9a-fA-F]{6}$/.test(color))) return msg.reply('Please give a color in hex format ex: #00ff00');
    const roles = roleManager.cache.filter(role=>/^#[0-9a-fA-F]{6}$/.test(role.name));
